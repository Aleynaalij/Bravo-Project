...
- **Bravo practice-area expansion (PRD §12):** the platform covered only Bravo's Purview-based Data Security & Compliance practice until now. Adds `cloud_migration`/`app_modernization`/`sharepoint`/`analytics_ai` services and one flagship deliverable per new practice area. Critically, the 10 templates above that were worded as if every project only ever has Purview services in scope (persona lines like "senior Microsoft 365 security and compliance consultant," HLD's "reference actual Purview capabilities by name") needed generalizing — otherwise a Cloud-Migration-only project would still get Purview-flavored Core docs. Shipped as version-2 rows (`supabase/migrations/0014_generalize_core_prompts.sql`), following the `prompt_templates` versioning design in `docs/ERD.md` (history kept for the `deliverable_versions` audit trail) rather than an in-place edit. HLD's `section_schema` gained 4 new `requires_service` design sections alongside the original 8, same pattern. **Not yet done:** prompt templates for the 4 new flagship deliverable types themselves (enum values exist; generating one today throws a clean `GenerationError` from `getActiveTemplate`, not a silent failure, but there's genuinely nothing to generate yet) and Knowledge Base content for the 4 new services (they currently retrieve zero KB entries).
