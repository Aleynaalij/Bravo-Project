# QuePilot.ai — Sprint 1 Implementation Plan
...
| E7 | Branding settings UI (firm name, logo, accent color) | ✅ Done | Now folded into `/dashboard/settings` (originally its own `/dashboard/branding` page). Server Action in `src/app/dashboard/branding/actions.ts` (not the `PUT /accounts/me/branding` route `docs/openapi.yaml` describes — same REST-surface-is-aspirational convention as the rest of this app). Logo/color now actually flow into all three export builders — previously only `firm_name_override` was consumed; see docs/TDD.md §2.6 |
| E8 | FileVault — cross-project view of every generated deliverable | ✅ Done | `/dashboard/vault`, `src/lib/vault/service.ts`. No new Storage/persistence — a gallery over the existing on-demand export pipeline (E4-E6), plus a built-in PDF viewer (`<iframe>`, native browser PDF rendering) and an Email action (`mailto:` with a link back into the app — no email-sending provider configured, see docs/TDD.md §2.6) |
...
- Multi-seat teams — ✅ **Done**, see "Multi-Seat Teams" section below. A distinct Partner/Enterprise billing tier on top of that remains not started.

## Practice-Area Expansion (catalog complete)

Mike asked the platform to cover everything sweepable from the design partner's public site, not just Purview — see PRD §12 for the full rationale and sourcing. This is a bigger scope change than anything in Sprint 2: the product covered exactly one of the design partner's ~5 practice areas until now.
...
PRD §11 flagged multi-seat as "a real gap for a firm this size, not just a Phase 2 nicety" — a ~46-person firm getting one individual account doesn't get the firm using the product. The `users`/`accounts` split was already designed for this back in Sprint 1 specifically so this wouldn't need a schema migration; it didn't.

- **Schema + RLS** — ✅ Done. `supabase/migrations/0017_multi_seat_teams.sql`: a `role` check constraint (`'owner' | 'member'`), a `SECURITY DEFINER` `auth_is_owner()` helper (same pattern as `auth_account_id()`/`is_platform_admin()`), one new RLS policy letting an owner update a teammate's role, and — the actual mechanism that makes invites work — `handle_new_auth_user()` now branches on `invited_account_id` in the new user's metadata: present and valid → join that account as `'member'`; absent → the original behavior, a brand-new account as `'owner'`. Every other account-scoped RLS policy already keyed off `auth_account_id()`, not a specific user, so a teammate gets full project/deliverable/branding access the moment their `users` row exists — no other policy changes needed. Applied directly to the live project; confirmed via the security advisor that `auth_is_owner()` shows the same accepted "signed-in users can call this SECURITY DEFINER function" finding already accepted for the other two helpers, not a new one.
- **Invite flow** — ✅ Done. Uses Supabase's own `admin.auth.admin.inviteUserByEmail()` — no separate email-sending provider needed, same transactional-email path signup/OAuth already rely on. `redirectTo` points at the existing `/auth/callback?next=/set-password`; no changes needed to that route, since it already redirects wherever `next` says once it exchanges the code for a session. New `/set-password` page reuses `changePasswordAction` rather than a separate code path.
- **Team management UI** — ✅ Done. A new "Team" section in `/dashboard/settings`: every member sees the full roster (read-only if they're not the owner), the owner additionally gets an invite form, a promote/demote toggle per teammate, and a confirm-gated Remove (which deletes the teammate's actual Supabase Auth user via the admin API, not just their `public.users` row — cascades to remove data access and login ability together). Guards against a zero-owner account: demoting or removing the last remaining owner is refused server-side.
- **Owner-gating** — ✅ Done. Billing (Stripe checkout/portal) and account deletion are now owner-only (`requireAccountOwner()`, new alongside the existing `requireAccountId()`/`requirePlatformAdmin()` in `src/lib/auth/session.ts`) — a teammate cancelling the whole account's subscription or deleting it out from under everyone else would be a real footgun now that more than one person can be signed into an account. Billing page UI hides the Subscribe/Manage-billing controls from non-owners (with a note) rather than just relying on the server-side check to silently no-op.
- No seat limit — same "no numeric limits without real usage data to size them against" decision already made for FR-16.
- ⚠️ Verified as far as this sandbox allows: the schema/RLS/trigger change was applied to and confirmed against the live database directly; the Team section's client-side logic (owner vs. member view, confirm-gating, correct form wiring) was verified via a throwaway preview route + Playwright. The actual invite round trip — email delivery, code exchange, the trigger branching on a real invited user, landing on `/set-password` — could not be exercised live, same network-egress block on the real Supabase host as every other write-path feature in `docs/validation-checklist.md`.
