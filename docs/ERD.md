...
**Approval workflow (migration 0044, the roadmap's "workflow engine" item):** `review_status` (`not_submitted | in_review | approved | changes_requested`) is a second lifecycle independent of `status` (the pending/generating/ready/failed generation pipeline) — it only means anything once `status = 'ready'`. `submitted_by`/`submitted_by_email`/`submitted_at` and `reviewed_by`/`reviewed_by_email`/`reviewed_at` record who moved it through the workflow and when, with the email columns denormalized (same "survives a deleted `users` row" rationale as every other `*_email` column in this schema — `knowledge_vault_entries.author_email`, `sops.author_email`, etc.) rather than requiring a join back to `users`. `review_note` holds the reviewer's explanation when requesting changes. Editing a deliverable (`saveEditedVersionAction`) resets every one of these fields back to `not_submitted`/null — a stale approval must not carry over onto content nobody has reviewed. The export route (`/api/projects/[id]/deliverables/[deliverableId]/export`) gates the default "attachment" (download) disposition behind `review_status = 'approved'`; the "inline" disposition (FileVault's own PDF preview, and this same view before it's approved) stays open regardless, since a reviewer has to be able to see a draft to approve it. Flat RBAC (no dedicated "reviewer" role) means any teammate can submit, approve, or request changes — including the same person who submitted it.
...
"Que," the conversational assistant on the dashboard home page (migration 0048) — a genuinely different shape from every other AI feature in this schema, which are all single-shot request-log tables (`eks_requests`, `automation_requests`). This one is real multi-turn conversation, so it gets actual conversation storage: `assistant_conversations` (one row per thread, `title` set once at creation from the first message so no update policy is needed) and `assistant_messages` (append-only, one row per turn, `role` discriminating `user`/`assistant`). `account_id` is denormalized directly onto `assistant_messages` too, not just reachable via the parent conversation — the same flat shape `eks_requests`/`automation_requests`/`content_views` all use, so both RLS and the rate limiter (`src/lib/assistant/rate-limit.ts`, 150/day — more generous than the Troubleshooting Engine's 50/day since this is the primary day-to-day surface) can query it directly without an ownership-via-parent-row join. `generated_sop_id` on a message is set when that assistant turn generated a real SOP ("generate an SOP for this"), letting the chat UI link straight to it without a second query. RLS is select+insert+delete on conversations (a user can clear one) and select+insert on messages, scoped to `account_id = auth_account_id()` on both — unlike `usage_events`, which is platform-admin-only, a firm's own team can see its own conversation history.

### content_views
Records who opened a `knowledge_vault_entries`/`knowledge_scripts`/`sops`/`playbooks` row (migration 0046, the "most valuable content" usage-tracking gap `docs/TDD.md` had left unscheduled) — a fundamentally different signal from every join table above, which all answer "was this cited by other content," never "did a person actually look at it." `content_type` discriminates the four tables (`vault_entry | script | sop | playbook`); `content_id` is deliberately **not** a foreign key, since it can point into any of the four depending on `content_type` — the same polymorphic-reference tradeoff `usage_events` already makes via untyped `jsonb` metadata, here via a typed-but-unconstrained column instead. `viewer_user_id` follows the same nullable/`on delete set null` + denormalized `viewer_email` pattern as every other actor-id column in this schema. The `(content_type, content_id, viewer_user_id, view_date)` unique index caps this at one row per viewer per piece of content per day — `recordContentView` (`src/lib/usage/content-views.ts`) is called unconditionally on every relevant page load and swallows the resulting unique-violation, so a "count once per viewer per day" contract holds without any session/cookie tracking. Unlike `usage_events` (admin-only select, since that table aggregates platform-wide billing-relevant signals), RLS here is select+insert for any account member — a firm's own team can see which of their own content is or isn't getting used, which is the entire point.
...
`coding_standards` follows the identical private, no-admin-override, all-four-verbs pattern. `automation_requests` is account-scoped, all four verbs as of migration 0043 — originally shipped select-plus-insert-only on the (wrong) assumption that it was a purely append-only call log like `audit_log`, but `runCodeCreator`/`runCodeAudit` both insert a row up front and then update it with `output`/`error_message` once the AI call settles, so that original policy set silently no-op'd every one of those updates under RLS (real bug, found investigating a Code Creator timeout, fixed by migration 0042's `automation_requests_update` policy). Migration 0043 then added a delete policy once Code Creator's request history UI needed one (`deleteCodeCreatorRequestAction`).
