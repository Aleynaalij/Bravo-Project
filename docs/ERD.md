...
### eks_requests
Shared request log (Expert Knowledge System Phase 2, migration 0034) for the EKS reasoning-layer family — Troubleshooting Engine, Architecture Advisor, and (migration 0048) **Ask Que**, the platform assistant (`feature` discriminates all three). Deliberately not a widened `automation_requests`: EKS reasoning calls optionally link back to a `project_id` and cross-reference Knowledge Vault entries, a usage pattern `automation_requests` doesn't have, so this gets its own table and its own rate limit (`src/lib/eks/rate-limit.ts`) rather than sharing either `automation_requests`' or `generation_jobs`' budget. Ask Que joining this table rather than getting its own is a deliberate design choice, not an oversight: it started as a genuinely different shape (a multi-turn chat with its own `assistant_conversations`/`assistant_messages` tables), but that design was walked back once its open-ended AI cost was flagged as a concern, and the revised single-shot version ("one question in, one grounded answer out") is structurally identical to how Troubleshooting Engine and Architecture Advisor already work — so it joins their table and shares their 50/day budget rather than inventing a fourth pattern. Same account-scoped RLS shape as `automation_requests` — see the RLS section below for a real bug (migration 0042) found and fixed in both tables' policies.
