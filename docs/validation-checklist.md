...
## Bravo practice-area expansion (this pass)

Mike asked for the platform to cover everything sweepable from bravocg.com, not just Purview (PRD §12). Batch 1 (schema/domain model/UI grouping) and batch 2 (generalizing the 10 previously-Purview-flavored prompts) are done; batch 3 (Knowledge Base content for the 4 new services) and batch 4 (prompt templates for the 4 new flagship deliverables) are not yet built.
...
- ✅ `supabase/migrations/0013` and `0014` verified directly via SQL: `pg_enum` shows 12 `service_type` and 18 `deliverable_type` values; `prompt_templates` shows all 10 generalized templates at version 2/`is_active=true` with version 1 correctly deactivated (not deleted — audit trail preserved per the ERD's versioning design).
- 🚫 The 4 new services (`cloud_migration`, `app_modernization`, `sharepoint`, `analytics_ai`) currently retrieve **zero** Knowledge Base entries — `getRelevantKnowledgeBaseEntries` is keyed to services in scope, and no KB content exists yet for these. A project scoped purely to e.g. Cloud Migration would generate its HLD/SOW/etc. with no grounding beyond the model's own knowledge. Real gap, not cosmetic — tracked for the next batch.
