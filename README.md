...
AI-powered SaaS platform for Microsoft consulting practices — security & compliance, cloud migration, app modernization, SharePoint, and analytics/AI. A consultant completes a guided intake questionnaire and QuePilot generates client-ready deliverables (executive summaries, SOWs, architecture designs, and more) in minutes instead of hours.
...
- [`docs/sprint-1-plan.md`](docs/sprint-1-plan.md) — Sprint 1 implementation plan (also tracks Sprint 2 progress at the bottom)
- [`docs/validation-checklist.md`](docs/validation-checklist.md) — what's been live-tested vs. built-and-DB-verified-only; work through this once we pause building
...
2. Apply the database schema: run every SQL file in `supabase/migrations/` in order against that Supabase project — via the Supabase SQL editor or `supabase db push` if using the CLI.
